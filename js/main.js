	...
	var Spirit = Block.subClass( function( config ){
...

		{
			...
			play: function( state, force ){
				...
				if ( this.jump_combo( state ) && state !== 'force_wait' ) return;
			

				if ( this.defense( state ) ) return; 
			...
			fireFrames: function(){
				...
				if ( !state ){   
					...
					this.keyManage.mirror( this.direction );  
					if ( this.statusManage.isCrouch()  ){  
						return this.play( 'stand_up', true );
					}else{
